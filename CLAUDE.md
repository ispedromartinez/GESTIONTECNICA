...
Frontend pages are vanilla HTML/JS files in the project root, served by `express.static(__dirname)`. The Authorization header is injected by a single global `window.fetch` monkey-patch in **`common.js`** (reads the token from `localStorage`/`sessionStorage`, no per-call wiring needed) — unified there on 2026-07-17 from what used to be two different mechanisms: `informe-preventivo.html` originally had this same patch inline, while `informe_clima_app.html`/`informe_wom_app.html` had a local `authH()` helper that had to be spread into every `fetch(...)` call by hand (easy to forget on a new call site). Other pages outside the three informe pages (`dashboard.html`, `panel_tecnico.html`, etc.) may still carry their own local `authH()` copy — not yet migrated to the shared interceptor.
